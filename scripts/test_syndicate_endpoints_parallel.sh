...
process_endpoints() {
  echo "Processing endpoints from $services_json"
  jq -rc '[.services[].endpoints[] as $endpoint | .members[] | {member: ., endpoint: $endpoint}] | .[]' "$services_json" |
  parallel -j 10 --colsep ' ' --env script_dir,gavel,jq,members_json \
  'bash -c '\''fetch_block_data {1} {2} {3} {4} {5} {6} {7}'\'' _ {service_name} {member} {network} {fqdn} $(rand 1 1000)'
}
